            ...
            while True:
                ...
                for item in products:
                    ...
                    if data_id:
                        ...
                        try:
                            # Use evaluate to click using JS, bypass overlays
                            await page.evaluate(f"document.querySelector('a.detail.html-btn[data-id=\"{data_id}\"]').click()")
                            
                            # Wait for modal content to be visible
                            await page.wait_for_selector('.modals-wrapper .modal-content, .mf-ajax-content', state='visible', timeout=10000)
                            await asyncio.sleep(2) # Buffer to ensure elements loaded
                            
                            modal_html = await page.content()
                            modal_soup = BeautifulSoup(modal_html, 'html.parser')
                            
                            modal_active = modal_soup.select_one('.modals-wrapper')
                            if modal_active:
                                desc = modal_active.select_one('.description, .product-description, [data-role="content"], .product.attribute.description .value')
                                if desc:
                                    desc_html = str(desc)
                                ...
                                spec_list = modal_active.select('table, ul.specs, .specifications')
                                if spec_list:
                                    specs_html = str(spec_list[0])
                                    specs['html'] = specs_html
                                    
                            # Close Modal via JS
                            await page.evaluate("document.querySelectorAll('.action-close').forEach(btn => btn.click())")
